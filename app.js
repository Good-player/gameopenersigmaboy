...
function App(){
  ...
  return(<div style={S.root}><style>{CSS}</style>
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",background:"#0d1117",border:"1px solid #151820",borderRadius:8,overflow:"hidden"}}>
          {!dmTo?<div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",color:"#555"}}>Select a conversation</div>:<>
            ...
            <div style={{padding:"8px 10px",borderTop:"1px solid #151820",display:"flex",gap:4}}>
              ...
              <button onClick={()=>{if(!dmMsg.trim())return;api("/dm/send",{username:account.username,token:account.token,to:dmTo,msg:dmMsg.trim()}).then(r=>{if(r?.ok){setDmMsg("");api("/dm/inbox",{username:account.username,token:account.token}).then(r2=>{if(r2?.ok)setDmInbox(r2)})}else{setToast({msg:r?.error||"Failed",color:"#eb4b4b"})}})}} style={{...S.btn,background:"#3b82f6",color:"#fff",padding:"6px 12px"}}>Send</button>
            </div>
          </>}
        </div>
      </div>}
    ...
    {giftModal&&<div style={S.overlay} onClick={()=>setGiftModal(null)}><div style={{...S.modal,maxWidth:340}} onClick={e=>e.stopPropagation()}>
      ...
      <input value={giftAmt} onChange={e=>setGiftAmt(e.target.value.replace(/[^0-9]/g,""))} placeholder="Amount in $" autoFocus style={{...S.input,width:"100%",fontSize:16,textAlign:"center"}} onKeyDown={async(e)=>{if(e.key==="Enter"){const amt=parseInt(giftAmt);if(!amt||amt<=0||amt>st.bal){setToast({msg:amt>st.bal?"Not enough":"Invalid amount",color:"#eb4b4b"});return}const r=await api("/gift",{username:account.username,token:account.token,to:giftModal.to,amount:amt,slot:slot});if(r?.ok){if(r.newBal!==undefined)setSt(p=>({...p,bal:r.newBal}));setToast({msg:"Sent $"+amt.toLocaleString()+"!",color:"#4ade80"});if(giftModal.context==="dm"){api("/dm/inbox",{username:account.username,token:account.token}).then(r2=>{if(r2?.ok)setDmInbox(r2)})}setGiftModal(null)}else setToast({msg:r?.error||"Failed",color:"#eb4b4b"})}}}/>
      ...
      <div style={{display:"flex",gap:6,width:"100%"}}>
        ...
        <button onClick={async()=>{const amt=parseInt(giftAmt);if(!amt||amt<=0||amt>st.bal){setToast({msg:amt>st.bal?"Not enough":"Invalid amount",color:"#eb4b4b"});return}const r=await api("/gift",{username:account.username,token:account.token,to:giftModal.to,amount:amt,slot:slot});if(r?.ok){if(r.newBal!==undefined)setSt(p=>({...p,bal:r.newBal}));setToast({msg:"Sent $"+amt.toLocaleString()+"!",color:"#4ade80"});if(giftModal.context==="dm"){api("/dm/inbox",{username:account.username,token:account.token}).then(r2=>{if(r2?.ok)setDmInbox(r2)})}setGiftModal(null)}else setToast({msg:r?.error||"Failed",color:"#eb4b4b"})}} style={{...S.btn,background:"#ffd700",color:"#000",flex:2,fontWeight:700}}>Send {giftAmt&&parseInt(giftAmt)>0?"$"+parseInt(giftAmt).toLocaleString():""}</button>
      </div>
    </div></div>}
