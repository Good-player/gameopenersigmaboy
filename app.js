...
function MI({n,s,c,f}){return <span className="material-icons-round" style={{fontSize:s||16,color:c||"inherit",verticalAlign:"middle",lineHeight:1,fontWeight:400,...(f||{})}}>{n}</span>}
...
function App(){
  ...
  const[feed,setFeed]=useState([]);const[lb,setLb]=useState([]);const[chatLog,setChatLog]=useState([]);const pfpCache=useRef({});const lastChatIdRef=useRef(0);const[chatMsg,setChatMsg]=useState("");const[nickname,setNickname]=useState(getUserName());const[nameEditing,setNameEditing]=useState(false);const[chatCd,setChatCd]=useState(0);
  ...
  async function joinLobby(id,pw){const r=await api("/lobby/join",{username:account?.username,lobbyId:id,lobbyPassword:pw||""});if(r?.ok){await refreshLobby(id);return true}else{alert(r?.error||"Failed");return false}}
  ...
  useEffect(()=>{(async()=>{
    ...
    try{const c=await getChat(true);if(c?.msgs){if(c.pfps)Object.entries(c.pfps).forEach(([uid,pfp])=>{pfpCache.current[uid]=pfp});chatPfpsLoaded=true;setChatLog(c.msgs.map(m=>({...m,pfp:pfpCache.current[m.uid]||""})))}}catch{}
  ...
  useEffect(()=>{if(showSlots)return;let on=true;let dmPollCount=0;async function poll(){if(!on)return;try{const[c,ref]=await Promise.all([getChat(!chatPfpsLoaded,chatPfpsLoaded?lastChatIdRef.current:0),api("/refresh",{})]);if(c?.msgs){if(c.pfps){Object.entries(c.pfps).forEach(([uid,pfp])=>{pfpCache.current[uid]=pfp});chatPfpsLoaded=true}const newMsgs=c.msgs.map(m=>({...m,pfp:pfpCache.current[m.uid]||""}));if(lastChatIdRef.current>0&&newMsgs.length>0){setChatLog(prev=>{const existIds=new Set(prev.map(m=>m.id));const fresh=newMsgs.filter(m=>!existIds.has(m.id));return[...fresh,...prev].slice(0,200)})}else{setChatLog(newMsgs)}if(newMsgs.length>0){const maxId=Math.max(...newMsgs.map(m=>m.id||0));if(maxId>lastChatIdRef.current)lastChatIdRef.current=maxId}};if(ref?.refreshAt&&ref.refreshAt>lastRefreshRef.current){lastRefreshRef.current=ref.refreshAt;location.reload()}dmPollCount++
        const ev2=await api("/events",{});if(ev2?.events)setEvents(ev2.events);if(dmPollCount%8===0){const[f,l]=await Promise.all([getFeed(),getLeaderboard()]);if(f?.feed)setFeed(f.feed);if(l?.lb)setLb(l.lb)}if(account?.username&&account?.token&&dmPollCount%2===0){try{const dm=await api("/dm/inbox",{username:account.username,token:account.token});if(dm?.ok&&dm.unread!==undefined){if(dm.unread>dmUnread&&dmUnread>=0){setToast({msg:dm.unread+" new message"+(dm.unread>1?"s":""),color:"#3b82f6"})}setDmUnread(dm.unread);if(dm.received)setDmInbox(prev=>({...prev,received:dm.received,sent:dm.sent||prev?.sent}))}}catch{}}if(account?.username&&account?.token&&dmPollCount%5===0){try{const st=await api("/status",{username:account.username,token:account.token});if(st?.banned){setBanModal({reason:st.banReason||"Banned",expires:st.banExpires||0});clearAccount();setAccount(null)}if(st?.warned&&!warnModal)setWarnModal(st.warnReason||"Warning");if(st?.forceSync&&st.forceSync>0){const lastSync=parseInt(localStorage.getItem("co-lastsync")||"0");if(st.forceSync>lastSync){localStorage.setItem("co-lastsync",String(st.forceSync));syncLockRef.current=true;try{const lr=await api("/auth/load",{username:account.username,token:account.token});if(lr?.ok&&lr.slots){for(let si=0;si<3;si++){if(lr.slots[si]){await _stSet("co-s"+si,JSON.stringify(lr.slots[si]))}}const curSlotData=lr.slots[slot];if(curSlotData&&curSlotData.st){setSt(s=>({...INIT,...curSlotData.st,starred:curSlotData.st?.starred||{}}));if(curSlotData.drops)setDrops(curSlotData.drops);setToast({msg:"Balance updated by admin",color:"#3b82f6"})}}}catch{}setTimeout(()=>{syncLockRef.current=false},3000)}}}catch{}}}catch{}};poll();const id=setInterval(poll,6000);return()=>{on=false;clearInterval(id)}},[showSlots,account]);
  ...
  useEffect(()=>{if(!account?.username||!account?.token||showSlots)return;const id=setInterval(()=>{cloudSave()},30000);return()=>clearInterval(id)},[account,showSlots,slot]);
  ...
  async function cloudSave(){
    if(!account||!account.username||!account.token)return;setSaveStatus("saving");
    try{
  ...
  async function cloudLoad(){
    if(!account)return;
    if(!confirm("Load slot "+(slot+1)+" from cloud? This will overwrite your local data."))return;
  ...
  return(<div style={S.root}><style>{CSS}</style>
    ...
    {/* Auth Modal */}
    {showAuth&&<div style={S.overlay} onClick={()=>setShowAuth(false)}><div style={S.modal} onClick={e=>e.stopPropagation()}>
    ...
    {/* HEADER */}
    <div style={S.hdr}><div style={{display:"flex",alignItems:"center",gap:"clamp(6px,1.5vw,10px)"}}><button onClick={()=>setShowSlots(true)} style={{...S.btn,background:"#ffffff08",color:"#888",padding:"clamp(4px,1vw,6px) clamp(6px,1.5vw,10px)",fontSize:"clamp(9px,2.2vw,11px)"}}>Slot {slot+1}</button><div style={S.title}>CASES</div></div><div style={S.hdrRight}><button onClick={()=>setShowAuth(true)} style={{...S.btn,background:account?"#4ade8022":"#ffffff08",color:account?"#4ade80":"#888",padding:"6px 12px",fontSize:"clamp(9px,2.2vw,12px)",border:account?"1px solid #4ade8033":"1px solid #333",fontWeight:700}}>{account?"✓ "+account.username:"Login"}</button>{account&&<><button onClick={cloudSave} style={{...S.btn,background:"#3b82f6",color:"#fff",padding:"6px 14px",fontSize:"clamp(10px,2.5vw,13px)",fontWeight:700}}><><MI n="cloud_upload" s={14}/> Save</></button><button onClick={cloudLoad} style={{...S.btn,background:"#8b5cf622",color:"#8b5cf6",padding:"6px 10px",fontSize:"clamp(9px,2.2vw,11px)"}}><><MI n="cloud_download" s={14}/> Load</></button></>}{saveStatus&&<span style={{color:saveStatus==="Saved!"?"#4ade80":"#fbbf24",fontSize:"clamp(9px,2.2vw,11px)",fontWeight:700,animation:"fadeUp .3s"}}>{saveStatus}</span>}<div style={S.chip}><span style={S.chipLbl}>BAL</span><span style={{color:st.bal<100?"#eb4b4b":"#4ade80",fontWeight:700,fontSize:"clamp(14px,3.5vw,20px)"}}>{money(st.bal)}</span></div>{st.loan>0&&<div style={{...S.chip,borderColor:"#eb4b4b33"}}><span style={S.chipLbl}>DEBT</span><span style={{color:"#eb4b4b",fontWeight:700,fontSize:"clamp(12px,3vw,18px)"}}>{money(st.loan)}</span></div>}<div style={S.rentChip}>Rent: {money(RENT_AMT)} in {rentIn} opens</div></div></div>
    ...
    {/* NAV */}
    <div style={S.nav}>{[["shop","Shop"],["inv",`Inv (${st.inv.length})`],["stats","Stats"],["loan","Loan"],["flip","Flip"],["pvp","PvP"],["live","Live"],["lb","Top"],["chat","Chat"],["dm",`DM${dmUnread>0?" ("+dmUnread+")":""}`],["me","Me"],["map","Map"],["horse","Horses"],["bj","BJ"],["btc","BTC"],["plinko","Plinko"],["roulette","Roulette"]].map(([k,l])=>(<button key={k} onClick={()=>{if(!opening||scrollDone||k==="shop"){setPage(k);if(k!=="opening")setOpening(false);if(k==="map")getOnline().then(r=>{if(r)setOnlineData(r)});if(k==="pvp")refreshLobbies();if(k==="bj"&&account){api("/bj/buycard",{username:account.username,token:account.token}).then(r=>{if(r?.hasCard)setBjHasCard(true)})}if(k==="btc"){/* useEffect handles initial load */}if(k==="dm"&&account){api("/dm/inbox",{username:account.username,token:account.token}).then(r=>{if(r?.ok){setDmInbox(r);setDmUnread(0);api("/dm/read",{username:account.username,token:account.token})}}).catch(()=>{setTimeout(()=>{api("/dm/inbox",{username:account.username,token:account.token}).then(r2=>{if(r2?.ok){setDmInbox(r2);setDmUnread(0)}})},2000)})}}}} style={{...S.tab,...(page===k||(page==="opening"&&k==="shop")?S.tabOn:{}),color:k==="dm"&&dmUnread>0?"#f59e0b":undefined}}>{l}</button>))}{account&&["admin","owner","mod"].includes(account.role)&&<button onClick={()=>{if(account.role==="owner")window.owner();else window.admin()}} style={{...S.tab,background:"#ff000022",color:"#ff4444",border:"1px solid #ff000044"}}>{account.role==="owner"?"Owner":"Admin"}</button>}<button onClick={()=>setConfirmReset(true)} style={{...S.tab,marginLeft:"auto",color:"#eb4b4b"}}>Reset</button></div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:3}}>
        ...
        lb.map((p,i)=>{const medal=null;return(
          <div key={i} style={{display:"grid",gridTemplateColumns:"28px 1fr 80px 65px",alignItems:"center",gap:8,padding:"8px 12px",background:i<3?"#ffd70008":"#0d1117",border:i<3?"1px solid #ffd70022":"1px solid #151820",borderRadius:6,fontSize:"clamp(9px,2.2vw,12px)",cursor:"pointer"}} onClick={()=>showProfile(p.uname)}>
            ...
            <span style={{fontWeight:600,color:"#e2e8f0",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{dName(p)} <span style={{color:"#666",fontSize:"clamp(7px,1.8vw,9px)"}}>Lv{p.level||1}</span></span>
    ...
    {page==="pvp"&&<div style={S.body}>
        ...
        <div style={{background:"#0d1117",border:"1px solid #151820",borderRadius:12,padding:"clamp(12px,3vw,20px)",marginBottom:10}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:2,marginBottom:10}}>
            {(curLobby._players||[]).map((p,i)=><div key={p.username} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 10px",background:i===0&&curLobby.status==="finished"?"#4ade8011":"#141820",borderRadius:6,borderLeft:"3px solid "+(p.username===curLobby.host?"#f59e0b":"#333")}}>
              ...
              <span style={{fontWeight:700,color:"#e2e8f0",flex:1,cursor:"pointer"}} onClick={()=>{api("/profile/full",{target:p.username,username:account?.username}).then(r=>{if(r?.profile)setViewProfile(r.profile)})}}>{p.username}</span>
      ...
      <>
        ...
        <div style={{background:"#0d1117",border:"1px solid #151820",borderRadius:12,padding:"clamp(12px,3vw,18px)",marginBottom:12}}>
          ...
          <button onClick={async()=>{const name=createLobbyName.trim()||account.username+"'s lobby";const r=await api("/lobby/create",{username:account.username,name,lobbyPassword:createLobbyPw,mode:createLobbyMode});if(r?.lobbyId){setCreateLobbyName("");setCreateLobbyPw("");await refreshLobby(r.lobbyId)}else{alert(r?.error||"Failed")}}} style={{...S.btn,background:"#4ade80",color:"#000",fontWeight:700,width:"100%"}}>Create Lobby</button>
        </div>
        {/* LOBBY LIST */}
        ...
        <div style={{display:"flex",flexDirection:"column",gap:4}}>
          ...
          lobbies.map(l=><div key={l.id} className="slideIn" style={{background:"#0d1117",border:"1px solid #151820",borderRadius:8,padding:"10px 14px",display:"flex",alignItems:"center",gap:10}}>
            <div style={{flex:1}}>
            ...
            {l.status==="waiting"&&<button onClick={async()=>{let pw="";if(l.password_hash){pw=prompt("Password:");if(pw===null)return}const ok=await joinLobby(l.id,pw);if(ok)await refreshLobby(l.id)}} style={{...S.btn,background:"#3b82f6",color:"#fff",padding:"6px 14px"}}>Join</button>}
      ...
      {!account?<div style={{color:"#f59e0b",padding:12,textAlign:"center"}}>Login required</div>:<div style={{display:"flex",gap:10,height:"clamp(300px,60vh,500px)"}}>
        ...
        <div style={{width:"clamp(100px,30vw,180px)",background:"#0d1117",border:"1px solid #151820",borderRadius:8,display:"flex",flexDirection:"column",flexShrink:0,overflow:"hidden"}}>
          <div style={{padding:"8px 10px",borderBottom:"1px solid #151820"}}>
            <div style={{display:"flex",gap:3}}><input value={dmSearch} onChange={e=>{setDmSearch(e.target.value.slice(0,20));const q=e.target.value.trim();if(q.length>=2){clearTimeout(window._dmST);window._dmST=setTimeout(()=>{api("/users/search",{q}).then(r=>{if(r?.users)setDmResults(r.users)})},600)}}} placeholder="Find user..." style={{...S.sel,flex:1,padding:"4px 6px",fontSize:10}} onKeyDown={e=>{if(e.key==="Enter"&&dmSearch.length>=2){api("/users/search",{q:dmSearch}).then(r=>{if(r?.users)setDmResults(r.users)})}}}/><button onClick={async()=>{if(dmSearch.length<2)return;const r=await api("/users/search",{q:dmSearch});if(r?.users)setDmResults(r.users)}} style={{...S.btn,background:"#3b82f6",color:"#fff",padding:"3px 6px",fontSize:9}}>Go</button></div>
      ...
      {!account?<div style={{color:"#f59e0b",padding:12,textAlign:"center"}}>Login required</div>:<>
        <div style={{background:"#0d1117",border:"1px solid #151820",borderRadius:12,padding:"clamp(14px,4vw,24px)",marginBottom:12}}>
          <div style={{display:"flex",gap:12,alignItems:"center",marginBottom:12}}>
            ...
            <div><div style={{fontWeight:700,fontSize:"clamp(14px,3.5vw,18px)",color:"#e2e8f0"}}>{account.username}</div><div style={{color:"#888",fontSize:"clamp(9px,2.2vw,11px)"}}>Level {account.level||1} &middot; {account.role||"user"}{account.user_number?" · #"+account.user_number:""}</div></div><button onClick={()=>{setPage("dm");if(account){api("/dm/inbox",{username:account.username,token:account.token}).then(r=>{if(r?.ok){setDmInbox(r);setDmUnread(0);api("/dm/read",{username:account.username,token:account.token})}})}}} style={{...S.btn,background:"#3b82f622",color:"#3b82f6",padding:"6px 10px",marginLeft:"auto",display:"flex",alignItems:"center",gap:4}}><MI n="chat" s={14}/>Messages{dmUnread>0?" ("+dmUnread+")":""}</button>
          </div>
    ...
    {page==="plinko"&&<div style={{...S.body,maxWidth:600,margin:"0 auto"}}>
      ...
      <div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:12,alignItems:"center",justifyContent:"center"}}>
        <input value={plinkoBet} onChange={e=>setPlinkoBet(e.target.value.replace(/[^0-9]/g,""))} placeholder="Bet" style={{...S.input,width:100,textAlign:"center"}}/>
        ...
        <button disabled={plinkoAnim} onClick={async()=>{const bet=parseInt(plinkoBet);if(!bet||bet<100||bet>st.bal){setToast({msg:"Invalid bet",color:"#eb4b4b"});return}setSt(p=>({...p,bal:p.bal-bet}));setPlinkoAnim(true);setPlinkoResult(null);const r=await api("/plinko/drop",{username:account?.username,token:account?.token,bet,rows:plinkoRows,risk:plinkoRisk});if(!r?.ok){setSt(p=>({...p,bal:p.bal+bet}));setPlinkoAnim(false);setToast({msg:r?.error||"Failed",color:"#eb4b4b"});return}
      ...
      {bjTable?(()=>{
        ...
        return <div>
          ...
          {canJoin&&<div style={{marginTop:12,padding:10,background:"#0d1117",borderRadius:8,textAlign:"center"}}>
            ...
            <div><button onClick={async()=>{const bet=parseInt(bjBet);if(!bet||bet<10000||bet>st.bal){setToast({msg:bet>st.bal?"Not enough":"Min $10K",color:"#eb4b4b"});return}setSt(p=>({...p,bal:p.bal-bet}));const r=await api("/bj/join",{username:account.username,token:account.token,tableId:"main",bet});if(r?.ok){setToast({msg:"Joined! Wait for round start",color:"#4ade80"});const s=await api("/bj/state",{tableId:"main",username:account.username});if(s?.table)setBjTable(s.table)}else{setSt(p=>({...p,bal:p.bal+bet}));setToast({msg:r?.error||"Failed",color:"#eb4b4b"})}}} style={{...S.btn,background:"#4ade80",color:"#000",padding:"8px 20px",fontWeight:700}}>Join with {money(parseInt(bjBet)||10000)}</button></div>
          </div>}
    ...
    {page==="btc"&&<div style={{...S.body,maxWidth:650,margin:"0 auto"}}>
      ...
      return <div>
        ...
        <div style={{display:"flex",gap:6,marginBottom:12,flexWrap:"wrap",alignItems:"center"}}>
          ...
          <button onClick={async()=>{const amt=parseInt(btcInvestAmt);if(!amt||amt<=0||amt>st.bal){setToast({msg:"Invalid",color:"#eb4b4b"});return}if(!account){setToast({msg:"Login required",color:"#eb4b4b"});return}setSt(p=>({...p,bal:p.bal-amt}));const r=await api("/btc/invest",{username:account.username,token:account.token,amount:amt,action:"buy"});if(r?.ok){setToast({msg:"Bought "+r.btcAmount.toFixed(6)+" BTC @ $"+r.buyPrice.toLocaleString(),color:"#4ade80"});save({...st,bal:st.bal-amt},drops);api("/btc/portfolio",{username:account.username,token:account.token}).then(r2=>{if(r2?.ok)setBtcPortfolio({active:r2.active||[],sold:r2.sold||[]})})}else{setSt(p=>({...p,bal:p.bal+amt}));setToast({msg:r?.error||"Failed",color:"#eb4b4b"})}}} style={{...S.btn,background:"#4ade80",color:"#000",padding:"8px 16px",fontWeight:700}}>Buy BTC</button>
        </div>
    ...
    {viewProfile&&<div style={S.overlay} onClick={()=>setViewProfile(null)}><div style={{...S.modal,maxWidth:"clamp(300px,85vw,440px)"}} onClick={e=>e.stopPropagation()}>
      ...
      <div style={{display:"flex",gap:4,width:"100%",flexWrap:"wrap"}}>
        {account&&viewProfile.username!==account.username&&<button onClick={async()=>{const r=await api("/friends/add",{username:account.username,token:account.token,target:viewProfile.username});if(r?.ok)alert(r.action==="accepted"?"Friend request accepted!":"Friend request sent!");else alert(r?.error||"Failed")}} style={{...S.btn,background:viewProfile.friendStatus==="friends"?"#4ade8022":"#3b82f622",color:viewProfile.friendStatus==="friends"?"#4ade80":"#3b82f6",flex:1,border:"1px solid "+(viewProfile.friendStatus==="friends"?"#4ade8033":"#3b82f633")}}>{viewProfile.friendStatus==="friends"?"Friends ✓":viewProfile.friendStatus==="sent"?"Pending...":viewProfile.friendStatus==="received"?"Accept ✓":"+ Add Friend"}</button>}
        ...
        {account&&viewProfile.username!==account.username&&<button onClick={()=>{const amt=prompt("Gift amount ($):");if(!amt||isNaN(parseInt(amt))||parseInt(amt)<=0)return;api("/gift",{username:account.username,token:account.token,to:viewProfile.username,amount:parseInt(amt),slot:slot}).then(r=>{if(r?.ok){if(r.newBal!==undefined){const ns={...st,bal:r.newBal};setSt(ns)}setToast({msg:"Sent $"+parseInt(amt).toLocaleString()+"!",color:"#4ade80"})}else{setToast({msg:r?.error||"Failed",color:"#eb4b4b"})}})}} style={{...S.btn,background:"#ffd70022",color:"#ffd700",flex:1,border:"1px solid #ffd70033"}}><MI n="redeem" s={14}/> Gift $</button>}
      ...
      <div style={{background:"#0d1117",border:"1px solid #151820",borderRadius:10,padding:"clamp(12px,3vw,20px)",marginBottom:12}}>
        ...
        {onlineData?.countries&&Object.entries(onlineData.countries).sort((a,b)=>b[1].count-a[1].count).map(([code,data])=>(
          <div key={code} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0",borderBottom:"1px solid #151820"}}>
            <span style={{fontSize:"clamp(16px,4vw,22px)"}}>{code!=="XX"?String.fromCodePoint(...[...code.toUpperCase()].map(c=>0x1F1E6+c.charCodeAt(0)-65)):"🌐"}</span>
...
const S={
  ...
  card:{background:"#0d1117",minWidth:0,border:"1px solid",borderRadius:12,padding:"clamp(10px,2.5vw,16px)",display:"flex",flexDirection:"column",alignItems:"center",gap:6},
