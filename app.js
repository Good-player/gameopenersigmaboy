...
if(dm.received)setDmInbox(prev=>({...prev,received:dm.received,sent:dm.sent||prev?.sent,unread:dm.unread}))}}}catch{}}if(account?.username&&account?.token&&dmPollCount%5===0){try{const st=await api("/status",{username:account.username,token:account.token});if(st?.banned){setBanModal({reason:st.banReason||"Banned",expires:st.banExpires||0});clearAccount();setAccount(null)}if(st?.warned&&!warnModal)setWarnModal(st.warnReason||"Warning");if(st?.forceSync&&st.forceSync>0){const lastSync=parseInt(localStorage.getItem("co-lastsync")||"0");if(st.forceSync>lastSync){localStorage.setItem("co-lastsync",String(st.forceSync));syncLockRef.current=true;try{const lr=await api("/auth/load",{username:account.username,token:account.token});if(lr?.ok&&lr.slots){for(let si=0;si<3;si++){if(lr.slots[si]){await _stSet("co-s"+si,JSON.stringify(lr.slots[si]))}}const curSlotData=lr.slots[slot];if(curSlotData&&curSlotData.st){setSt(s=>({...INIT,...curSlotData.st,starred:curSlotData.st?.starred||{}}));if(curSlotData.drops)setDrops(curSlotData.drops);setToast({msg:"Balance updated by admin",color:"#3b82f6"})}}}catch{}setTimeout(()=>{syncLockRef.current=false},3000)}}}catch{}}}catch{}};poll();const id=setInterval(poll,6000);return()=>{on=false;clearInterval(id)}},[showSlots,account]);
  ...
  async function doOpen(c){
    ...
    setTimeout(()=>{setScrollDone(true);const isEpicDrop=["covert","chroma","legendary"].includes(winner.rarity);const isHugeWin=winner.value>=c.price*5;if(isEpicDrop){document.body.classList.add("shakeHard");setTimeout(()=>document.body.classList.remove("shakeHard"),500);
      // Particle burst on legendary/chroma
  ...
  return(<div style={S.root}><style>{CSS}</style>
          ...
          {curLobby.mode==="buckshot"&&<div style={{...S.buckshotPanel,marginBottom:10,minHeight:curLobby.status==="playing"?"calc(100vh - 200px)":"auto",maxWidth:curLobby.status==="playing"?"none":undefined}}>
            ...
            {curLobby.status==="playing"&&buckshotState&&(()=>{
              ...
              return <div style={{position:"relative"}}>
                ...
                {/* Item legend (mobile-friendly tooltip alternative) */}
                {(buckshotState.round||1)>=2&&<div style={{background:"#0a0404",border:"1px solid #3a1818",borderRadius:4,padding:8,marginTop:8,fontFamily:"'Special Elite',serif",fontSize:10,color:"#94a3b8"}}>
                  <div style={{color:"#666",fontSize:9,marginBottom:4,letterSpacing:1}}>ITEMS</div>
    ...
    {/* PROFILE VIEW MODAL */}
    {buckshotConfirm&&<div style={S.overlay} onClick={()=>setBuckshotConfirm(null)}><div style={{...S.modal,maxWidth:400,padding:24,background:"linear-gradient(180deg,#1a0808,#0a0404)",border:"2px solid #5a1a1a",fontFamily:"'Special Elite',serif"}} onClick={e=>e.stopPropagation()}>
