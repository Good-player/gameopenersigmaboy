...
function App(){
  ...
  useEffect(()=>{
    ...
    const handler=(e)=>{
      ...
      if(navEl&&!navEl.contains(e.target))setOpenCategory(null);
    };
  ...
  useEffect(()=>{
    ...
    const unsubDm=window.RT.on("dm",(d)=>{
      if(!d||!d.from_user||d.from_user==="undefined"||!d.msg)return;
      if(d.unread!==undefined)setDmUnread(d.unread);
      if(d.from_user!==account?.username){
...
if(dm.received)setDmInbox(prev=>({...prev,received:recFiltered,sent:sentFiltered||prev?.sent,unread:dm.unread}))}}}catch{}}if(account?.username&&account?.token&&dmPollCount%5===0){try{const st=await api("/status",{username:account.username,token:account.token});if(st?.banned){setBanModal({reason:st.banReason||"Banned",expires:st.banExpires||0});clearAccount();setAccount(null)}if(st?.warned&&!warnModal)setWarnModal(st.warnReason||"Warning");if(st?.forceSync&&st.forceSync>0){const lastSync=parseInt(localStorage.getItem("co-lastsync")||"0");const isFresh=(Date.now()-st.forceSync)<600000; /* 10min */ if(st.forceSync>lastSync&&isFresh){localStorage.setItem("co-lastsync",String(st.forceSync));syncLockRef.current=true;try{const lr=await api("/auth/load",{username:account.username,token:account.token,ackForceSync:true});if(lr?.ok&&lr.slots){for(let si=0;si<3;si++){if(lr.slots[si]){await _stSet("co-s"+si,JSON.stringify(lr.slots[si]))}}const curSlotData=lr.slots[slot];if(curSlotData&&curSlotData.st){setSt(s=>({...INIT,...curSlotData.st,starred:curSlotData.st?.starred||{}}));if(curSlotData.drops)setDrops(curSlotData.drops);setToast({msg:"Synced from cloud",color:"#3b82f6"})}}}catch{}setTimeout(()=>{syncLockRef.current=false},3000)}else if(st.forceSync>0&&!isFresh){/* Stale flag — clear it without overwriting local */ try{await api("/auth/load",{username:account.username,token:account.token,ackForceSync:true,clearOnly:true})}catch{}; localStorage.setItem("co-lastsync",String(st.forceSync))}}}catch{}}}catch{}};poll();let pollTimer=null;function schedulePoll(){if(!on)return;const ms=(window.RT&&window.RT.isLive())?20000:6000;pollTimer=setTimeout(async()=>{await poll();schedulePoll()},ms)}schedulePoll();return()=>{on=false;clearTimeout(pollTimer)}},[showSlots,account]);
  ...
  useEffect(()=>{
    ...
    const onDm=(data)=>{
      if(!data||!data.from_user||data.from_user==="undefined"||!data.msg)return;
      if(account?.username&&data.from_user.toLowerCase()===account.username.toLowerCase())return;
  ...
  // Navigate to a page, running any on-enter loaders the destination needs.
  // Extracted from the old inline tab handler so categorized tabs and dropdowns can share it.
  const goTo=(k)=>{
    if(opening&&!scrollDone&&k!=="shop")return;
    setPage(k);
    if(k!=="opening")setOpening(false);
    ...
    if(k==="school"){
      if(!weatherData)api("/weather/today",{lat:59.4288,lon:17.9498}).then(r=>setWeatherData(r));
      if(!smhiWarnings)api("/weather/warnings",{}).then(r=>setSmhiWarnings(r||{warnings:[]}));
      if(!pollenData)api("/weather/pollen",{}).then(r=>setPollenData(r||{today:[]}));
      if(!airData)api("/weather/air",{lat:59.4288,lon:17.9498}).then(r=>setAirData(r));
    }
    if(k==="dm"&&account){
      api("/dm/inbox",{username:account.username,token:account.token}).then(r=>{
        if(r?.ok){
          setDmInbox(r);setDmUnread(0);
          const mx=Math.max(0,...(r.received||[]).map(m=>m.id||0),...(r.sent||[]).map(m=>m.id||0));
          if(mx>lastDmIdRef.current)lastDmIdRef.current=mx;
          api("/dm/read",{username:account.username,token:account.token});
        }
      }).catch(()=>{
        setTimeout(()=>{
          api("/dm/inbox",{username:account.username,token:account.token}).then(r2=>{
            if(r2?.ok){
              setDmInbox(r2);setDmUnread(0);
              const mx=Math.max(0,...(r2.received||[]).map(m=>m.id||0),...(r2.sent||[]).map(m=>m.id||0));
              if(mx>lastDmIdRef.current)lastDmIdRef.current=mx;
            }
          });
        },2000);
  ...
  return(<div style={S.root}><style>{CSS}</style>
    ...
    {!(curLobby?.mode==="buckshot"&&curLobby?.status==="playing")&&(()=>{
      ...
      const tabLabel=(k)=>{
        ...
        if(k==="inv")return `${t("tab_inv")} (${st.inv.length})`;
      ...
      const categoryOn=(cat)=>inCategory[cat].includes(page);
        ...
        <div style={{background:"#0d1117",border:"1px solid #151820",borderRadius:12,padding:"clamp(12px,3vw,20px)",marginBottom:10}}>
          ...
          {curLobby.mode==="buckshot"&&<div style={{...S.buckshotPanel,marginBottom:10,minHeight:curLobby.status==="playing"?"calc(100vh - 200px)":"auto",maxWidth:curLobby.status==="playing"?"none":undefined}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
              ...
              <div key={"pot-"+(curLobby.pot||0)} className="potChange" style={{fontFamily:"'Black Ops One',sans-serif",color:"#ffd700",fontSize:16,letterSpacing:2,textShadow:"0 0 6px #ffd70044",display:"inline-block"}}>POT: {money(curLobby.pot||0)}</div>
            </div>
      ...
      {!account?<div style={{color:"#f59e0b",padding:12,textAlign:"center"}}>Login required</div>:<div style={{display:"flex",gap:10,height:"clamp(300px,60vh,500px)"}}>
        ...
        <div style={{width:"clamp(100px,30vw,180px)",background:"#0d1117",border:"1px solid #151820",borderRadius:8,display:"flex",flexDirection:"column",flexShrink:0,overflow:"hidden"}}>
          ...
          <div style={{flex:1,overflowY:"auto"}}>
            {(()=>{const convos={};(dmInbox?.received||[]).forEach(m=>{if(m&&m.from_user&&m.from_user!=="undefined"&&(!account?.username||m.from_user.toLowerCase()!==account.username.toLowerCase())){if(!convos[m.from_user])convos[m.from_user]={user:m.from_user,lastMsg:m.msg,ago:m.ago,unread:!m.read,pfp:m.pfp||""}}});(dmInbox?.sent||[]).forEach(m=>{if(m&&m.to_user&&m.to_user!=="undefined"&&(!account?.username||m.to_user.toLowerCase()!==account.username.toLowerCase())){if(!convos[m.to_user])convos[m.to_user]={user:m.to_user,lastMsg:m.msg,ago:m.ago,pfp:m.to_pfp||""}}});return Object.values(convos).filter(c=>c&&c.user&&c.user!=="undefined").map(c=><button key={c.user} onClick={()=>setDmTo(c.user)} style={{display:"flex",gap:6,alignItems:"center",padding:"8px 10px",background:dmTo===c.user?"#1e2430":"transparent",border:"none",borderBottom:"1px solid #0a0c10",cursor:"pointer",width:"100%",fontFamily:"inherit"}}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
            <div style={{display:"flex",alignItems:"center",gap:14}}>
              ...
              <div>
                ...
                {c.feels_like!==undefined&&Math.abs((c.feels_like||0)-(c.temp||0))>0.5&&<div style={{fontSize:11,color:c.feels_like<c.temp?"#60a5fa":"#fb923c",marginTop:2,display:"flex",alignItems:"center",gap:3,fontWeight:600}}>{lang==="sv"?"Känns som":"Feels like"} {Math.round(c.feels_like)}°</div>}
                {c.feels_like!==undefined&&Math.abs((c.feels_like||0)-(c.temp||0))<=0.5&&<div style={{fontSize:10,color:"#888",marginTop:2}}>{lang==="sv"?"Känns som":"Feels like"} {Math.round(c.feels_like)}°</div>}
          ...
          <div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:6,scrollSnapType:"x mandatory"}}>
            {w.hourly.slice(0,expandedSection.hr?24:12).map((h,i)=>{
              ...
              return <div key={i} style={{flexShrink:0,textAlign:"center",background:isFirst?"linear-gradient(180deg,#1e3a8a44,#0d1117)":"#080a0e",borderRadius:10,padding:"10px 12px",minWidth:72,border:isFirst?"1px solid #3b82f655":"1px solid #1e2430",scrollSnapAlign:"start",animation:"fadeUp .4s",animationDelay:(i*0.03)+"s",animationFillMode:"backwards"}}>
                ...
                {h.feels_like!==undefined&&Math.abs((h.feels_like||0)-(h.temp||0))>=1.5&&<div style={{fontSize:9,color:h.feels_like<h.temp?"#60a5fa":"#fb923c",lineHeight:1}}>~{Math.round(h.feels_like)}°</div>}
    ...
    {/* TOAST */}
    {toast&&<div style={{position:"fixed",top:20,left:"50%",transform:"translateX(-50%)",zIndex:99999,pointerEvents:"none"}}><div className="toastIn" style={{background:toast.color+"22",border:"1px solid "+toast.color+"44",color:toast.color,padding:"10px 20px",borderRadius:8,fontWeight:700,fontSize:"clamp(10px,2.5vw,13px)",backdropFilter:"blur(8px)",boxShadow:"0 6px 24px "+toast.color+"33"}}>{toast.msg}</div></div>}
    {userMenu&&(()=>{
